...
@php
    ...
    // Font seragam, hanya top yang bergeser sesuai jumlah baris nama
    $nameTop  = match($lineCount) { 1 => '106px', 2 => '100px', default => '93px' };
    $niaTop   = match($lineCount) { 1 => '120px', 2 => '130px', default => '140px' };
    $validTop = match($lineCount) { 1 => '133px', 2 => '143px', default => '153px' };
@endphp
...
<div style="width:340px;height:194px;border-radius:6px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.18);position:relative;background:#ddeeff;">
    ...
    {{-- Foto: kiri, top sejajar nama --}}
    <div style="position:absolute;left:10px;top:{{ $nameTop }};width:68px;height:84px;overflow:hidden;border-radius:3px;border:1px solid #b0bac5;">
        @if ($member->photo)
