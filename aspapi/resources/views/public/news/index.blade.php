...
<section class="bg-white border-b border-neutral-200 sticky top-0 z-30">
    <div class="max-w-7xl mx-auto px-6 py-4">
        <form method="GET" action="{{ route('news.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('news.index') }}" class="text-xs text-neutral-400 hover:text-accent-red transition-colors">Reset ×</a>
            @endif
        </form>
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
            ...
            <span class="font-bold {{ $news->total() > 0 ? 'text-primary' : 'text-neutral-400' }}">{{ $news->total() }}</span>
        ...
        @else
            ...
            <div class="mt-12 flex flex-col items-center gap-2">
                ...
                <p class="text-xs text-neutral-400">
                    Menampilkan {{ $news->firstItem() }}–{{ $news->lastItem() }}
                    dari {{ $news->total() }} hasil
                    @if(!empty($keywords))
                        pencarian (total {{ $totalCount }} berita)
                    @else
                        berita
                    @endif
                </p>
