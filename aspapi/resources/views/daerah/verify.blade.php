                ...
                <td style="padding:0.875rem 1rem;">
                    <div style="display:flex;flex-wrap:wrap;gap:0.375rem;">
                        @if ($member->biodata_status === 'pending')
                            @if ($member->claims_old_member)
                                <form method="POST" action="{{ route('daerah.verify.approve-old', $member->id) }}">
                                    @csrf
                                    <button type="submit"
                                            ...
                                            title="Verifikasi biodata sekaligus konfirmasi anggota lama sejak {{ $member->claimed_join_year }}">
                                        ✓ Approve (Lama {{ $member->claimed_join_year }})
                                    </button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('daerah.verify.approve', $member->id) }}">
                                    @csrf
                                    <button type="submit"
...
function showRejectModal(memberId) {
    document.getElementById('reject-form').action = '/daerah/verifikasi/' + memberId + '/reject';
