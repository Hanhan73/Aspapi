...
<div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1rem 1.25rem;margin-bottom:1.25rem;">
    <form method="GET" action="{{ route('bendahara.payments') }}"
        style="display:flex;gap:0.75rem;flex-wrap:wrap;align-items:center;">

        {{-- Search --}}
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama / email anggota..."
        ...
        <select name="type"
            ...
            <option value="uang_pangkal" {{ request('type') === 'uang_pangkal'  ? 'selected' : '' }}>Uang Pangkal
            </option>
            <option value="iuran_tahunan" {{ request('type') === 'iuran_tahunan' ? 'selected' : '' }}>Iuran Tahunan
            </option>
        </select>
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="background:#EEF4FB;">
                    <th
                        style="padding:0.75rem 1rem;text-align:left;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2A7FC1;white-space:nowrap;">
                        Anggota</th>
                    <th
                        style="padding:0.75rem 1rem;text-align:left;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2A7FC1;white-space:nowrap;">
                        Jenis</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #EEF4FB;" onmouseover="this.style.background='#F8FAFC'"
                    ...
                    <td style="padding:0.875rem 1rem;font-size:0.825rem;color:#4A6580;white-space:nowrap;">
                        {{ $pay->type_label }}
                    </td>
                    <td
                        style="padding:0.875rem 1rem;font-size:0.825rem;font-weight:700;color:#1A2A3A;white-space:nowrap;">
                    ...
                    <td style="padding:0.875rem 1rem;">
                        ...
                        @endphp
                        <span
                            style="font-size:0.65rem;font-weight:700;padding:0.2rem 0.5rem;border-radius:2px;white-space:nowrap;{{ $stStyle }}">
                            {{ $stLabel }}
                        </span>
                    </td>
                    <td style="padding:0.875rem 1rem;font-size:0.8rem;color:#4A6580;white-space:nowrap;">
                    ...
                    <td style="padding:0.875rem 1rem;">
                        ...
                        @endphp
                        @if ($receipt)
                        <a href="{{ route('bendahara.kwitansi', $receipt->id) }}" target="_blank"
                            style="font-size:0.65rem;font-weight:700;padding:0.25rem 0.5rem;background:#2A7FC1;color:#fff;border-radius:3px;text-decoration:none;white-space:nowrap;">
                            🧾 {{ $receipt->receipt_number }}
                        </a>
                        @else
                        <span style="font-size:0.65rem;color:#B0CCDF;white-space:nowrap;">Menunggu pasangan</span>
                        @endif
                        @endif
                    </td>
                </tr>
...
<div id="reject-pay-modal"
    ...
    <div style="background:#fff;border-radius:8px;padding:2rem;width:400px;max-width:90vw;">
        <h3 style="font-family:'DM Serif Display',serif;font-size:1.25rem;color:#1A2A3A;margin-bottom:1rem;">Tolak
            Pembayaran</h3>
        <form id="reject-pay-form" method="POST">
            @csrf
            <label
                style="display:block;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#4A6580;margin-bottom:0.5rem;">Alasan
                *</label>
