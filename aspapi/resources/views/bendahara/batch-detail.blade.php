...
<div style="display:grid;grid-template-columns:1fr 320px;gap:1.25rem;align-items:start;">
    ...
    {{-- ── KIRI: Bukti Transfer + Daftar Anggota ── --}}
    <div style="display:flex;flex-direction:column;gap:1.25rem;">

        {{-- Bukti Transfer --}}
        <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;overflow:hidden;">
            <div style="padding:0.875rem 1.25rem;border-bottom:1px solid #EEF4FB;display:flex;align-items:center;justify-content:space-between;">
                <p style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#2A7FC1;margin:0;">Bukti Transfer</p>
            ...
            <div style="padding:1rem;background:#F8FAFC;">
                ...
                @if (in_array(strtolower($ext), ['jpg','jpeg','png','webp']))
                    <img src="{{ Storage::url($batch->receipt_path) }}"
                         alt="Bukti Transfer"
                ...
                @elseif (strtolower($ext) === 'pdf')
                    <iframe src="{{ Storage::url($batch->receipt_path) }}"
                            style="width:100%;height:520px;border:none;border-radius:4px;"></iframe>
                @else
                ...
                <table style="width:100%;border-collapse:collapse;">
                    <thead>
                        <tr style="background:#EEF4FB;">
                            <th style="padding:0.625rem 1rem;text-align:left;font-size:0.65rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2A7FC1;">Anggota</th>
                            <th style="padding:0.625rem 1rem;text-align:left;font-size:0.65rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2A7FC1;">NIA</th>
                            ...
                            <th style="padding:0.625rem 1rem;text-align:left;font-size:0.65rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2A7FC1;">Status Akun</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid #EEF4FB;" onmouseover="this.style.background='#F8FAFC'" onmouseout="this.style.background='#fff'">
                            <td style="padding:0.75rem 1rem;">
                                <p style="font-size:0.85rem;font-weight:600;color:#1A2A3A;margin:0;">{{ $payment->member->full_name ?? '—' }}</p>
                                <p style="font-size:0.72rem;color:#B0CCDF;margin:0;">{{ $payment->member->email ?? '' }}</p>
                            </td>
                            ...
                            <td style="padding:0.75rem 1rem;">
                                ...
                                @if ($s === 'active')
                                    <span style="font-size:0.65rem;font-weight:700;padding:0.2rem 0.5rem;border-radius:2px;background:#F0FFF4;color:#276749;">Aktif</span>
                                @else
                                    <span style="font-size:0.65rem;font-weight:700;padding:0.2rem 0.5rem;border-radius:2px;background:#F5F0FF;color:#9B59B6;">Belum Aktif</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
    ...
    {{-- ── KANAN: Info Batch + Aksi ── --}}
    <div style="display:flex;flex-direction:column;gap:1rem;">

        {{-- Info Batch --}}
        <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1.25rem;">
            <p style="font-size:0.65rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#4A6580;margin-bottom:1rem;">Info Batch</p>
            ...
            @php
                ...
                $stLabel = $st === 'verified' ? 'Terverifikasi' : ($st === 'rejected' ? 'Ditolak' : 'Pending');
            @endphp
            ...
            <div style="display:flex;flex-direction:column;gap:0.75rem;font-size:0.82rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <span style="font-size:0.65rem;font-weight:700;padding:0.2rem 0.6rem;border-radius:2px;{{ $stStyle }}">{{ $stLabel }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;">
                    <span style="color:#4A6580;">Wilayah</span>
                    <span style="font-weight:600;color:#1A2A3A;">{{ $batch->region->province ?? '—' }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;">
                    <span style="color:#4A6580;">Diajukan Oleh</span>
                    <span style="font-weight:600;color:#1A2A3A;text-align:right;max-width:140px;">{{ $batch->submitter->name ?? '—' }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;">
                ...
                <div style="display:flex;justify-content:space-between;border-top:1px solid #EEF4FB;padding-top:0.75rem;margin-top:0.25rem;">
                    <span style="color:#4A6580;">Total</span>
                    <span style="font-weight:700;color:#1A2A3A;font-size:0.9rem;">Rp {{ number_format($batch->total_amount, 0, ',', '.') }}</span>
                </div>
        ...
        <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1.25rem;">
            <p style="font-size:0.65rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#4A6580;margin-bottom:0.875rem;">Aksi</p>

            <form method="POST" action="{{ route('bendahara.batch.verify', $batch->id) }}">
                @csrf
                <button type="submit"
                        style="width:100%;padding:0.625rem;background:#276749;color:#fff;border:none;border-radius:4px;font-size:0.75rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;margin-bottom:0.625rem;">
                    ✓ Verifikasi Semua Anggota
                </button>
            </form>
            ...
            <div id="reject-form" class="hidden" style="margin-top:0.875rem;">
                <form method="POST" action="{{ route('bendahara.batch.reject', $batch->id) }}">
                    @csrf
                    <textarea name="reason" required placeholder="Alasan penolakan..."
                              style="width:100%;padding:0.625rem;border:1.5px solid #D6E8F7;border-radius:4px;font-size:0.8rem;color:#1A2A3A;outline:none;resize:vertical;min-height:80px;box-sizing:border-box;"></textarea>
                    <button type="submit"
                            style="width:100%;margin-top:0.5rem;padding:0.5rem;background:#C0392B;color:#fff;border:none;border-radius:4px;font-size:0.75rem;font-weight:700;cursor:pointer;">
                        Kirim Penolakan
                    </button>
                </form>
            </div>
