...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
    <div>
        ...
        <h2 style="font-family:'DM Serif Display',serif;font-size:1.5rem;color:#1A2A3A;margin-top:0.125rem;">Dewan Penasihat</h2>
    </div>
            ...
            <tr style="border-bottom:1px solid #EEF4FB;" onmouseover="this.style.background='#F8FAFC'" onmouseout="this.style.background='#fff'">
                <td style="padding:0.875rem 1rem;">
                    <div style="display:flex;align-items:center;gap:0.75rem;">
                        @if ($advisor->photo)
                        <img src="{{ Storage::url($advisor->photo) }}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;border:2px solid #D6E8F7;flex-shrink:0;"/>
                        @else
                        <div style="width:40px;height:40px;border-radius:50%;background:#EEF4FB;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            <span style="font-size:0.9rem;font-weight:700;color:#2A7FC1;">{{ strtoupper(substr($advisor->name, 0, 1)) }}</span>
                        </div>
                        @endif
                        <p style="font-size:0.85rem;font-weight:600;color:#1A2A3A;">{{ $advisor->name }}</p>
                    </div>
                </td>
                <td style="padding:0.875rem 1rem;font-size:0.825rem;color:#4A6580;">{{ $advisor->position ?? '—' }}</td>
                <td style="padding:0.875rem 1rem;font-size:0.825rem;color:#4A6580;">{{ $advisor->institution ?? '—' }}</td>
                <td style="padding:0.875rem 1rem;">
                    <span style="font-size:0.65rem;font-weight:700;padding:0.2rem 0.5rem;border-radius:2px;
                        {{ $advisor->is_active ? 'background:#F0FFF4;color:#276749;' : 'background:#FEF8EC;color:#B8860B;' }}">
                        {{ $advisor->is_active ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </td>
                <td style="padding:0.875rem 1rem;">
                    <div style="display:flex;gap:0.5rem;">
                        ...
                        <form method="POST" action="{{ route('admin.advisors.destroy', $advisor) }}" onsubmit="return confirm('Hapus?')">
                            @csrf @method('DELETE')
...
<div style="margin-top:1rem;display:flex;justify-content:flex-end;">{{ $advisors->links() }}</div>
@endif
@endsection