@php $isEdit = isset($advisor); $item = $advisor ?? null; @endphp
    ...
    <div style="display:flex;flex-direction:column;gap:1.25rem;">
        ...
        <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1.25rem;display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $item->title ?? '') }}" placeholder="Prof., Dr., dst."
            ...
            <div>
                <label style="display:block;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#4A6580;margin-bottom:0.5rem;">Jabatan dalam Dewan</label>
                <input type="text" name="position" value="{{ old('position', $item->position ?? '') }}" placeholder="Anggota, Ketua, dst."
                       style="width:100%;padding:0.625rem 0.875rem;border:1.5px solid #D6E8F7;border-radius:4px;font-size:0.875rem;color:#1A2A3A;outline:none;box-sizing:border-box;"
                       onfocus="this.style.borderColor='#2A7FC1'" onblur="this.style.borderColor='#D6E8F7'"/>
            </div>
        </div>
    ...
    <div style="display:flex;flex-direction:column;gap:1.25rem;">
        ...
        <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1.25rem;">
            ...
            <input type="file" name="photo" accept="image/*"
                   ...
                   onchange="previewAdvisorPhoto(this)"/>
        </div>
    </div>
</div>
<script>
function previewAdvisorPhoto(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = e => {
            document.getElementById('advisor-photo-preview').innerHTML =
