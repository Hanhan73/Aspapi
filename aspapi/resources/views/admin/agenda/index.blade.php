...
<div id="modal-detail"
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 py-4 border-t border-neutral-100 flex gap-3 flex-shrink-0">
            ...
            {{-- Tombol Setujui (muncul kalau pending) --}}
            <form id="modal-approve-form" method="POST" action="" class="flex-1" style="display:none;">
                @csrf
                <button type="submit"
                        class="w-full btn justify-center"
                        style="background:#D1FAE5;color:#065F46;font-weight:700;">
                    ✓ Setujui
                </button>
            </form>

            {{-- Tombol Tolak (muncul kalau pending) --}}
            <button id="modal-reject-btn"
                    type="button"
                    onclick="openRejectFromModal()"
                    class="flex-1 btn justify-center"
                    style="display:none;background:#FEE2E2;color:#991B1B;font-weight:700;">
                ✕ Tolak
            </button>
        </div>
    </div>
...
<div id="modal-reject-form"
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        ...
        <p id="reject-modal-title" class="text-xs text-neutral-400 mb-4"></p>
        <form id="reject-form" method="POST">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit"
                        class="flex-1 btn text-white hover:opacity-90"
...
function openDetailModal(id) {
    ...
    currentAgendaId    = id;
    currentAgendaTitle = el.dataset.title;
...
function openRejectModal(id) {
    currentAgendaId    = id;
    const el           = document.getElementById('agenda-data-' + id);
    currentAgendaTitle = el ? el.dataset.title : '';
    document.getElementById('reject-modal-title').textContent = currentAgendaTitle;
...
function openRejectFromModal() {
    // Tutup modal detail, buka modal tolak
    closeDetailModal();
    setTimeout(() => openRejectModal(currentAgendaId), 100);
}
