...
<div style="max-width:560px;">
    ...
    <div style="margin-bottom:1.5rem;">
        ...
        <h1 style="font-family:'DM Serif Display',serif;font-size:1.5rem;color:#1A2A3A;margin:0.5rem 0 0;">Tambah Akun</h1>
    </div>
    ...
    <div style="background:#fff;border:1px solid #D6E8F7;border-radius:6px;padding:1.5rem;">
        <form method="POST" action="{{ route('admin.superadmin.store') }}">
            ...
            {{-- Role --}}
            <div style="margin-bottom:1rem;" x-data="{ role: '{{ old('role') }}' }">
                ...
                <select name="role" x-model="role" required
                    ...
                    <option value="admin"         {{ old('role') === 'admin'         ? 'selected' : '' }}>Admin (Ketua/Sekjen)</option>
                    <option value="bendahara"     {{ old('role') === 'bendahara'     ? 'selected' : '' }}>Bendahara</option>
                    <option value="aspapi_daerah" {{ old('role') === 'aspapi_daerah' ? 'selected' : '' }}>ASPAPI Daerah</option>
                </select>
                @error('role')<p style="font-size:0.75rem;color:#C0392B;margin-top:0.25rem;">{{ $message }}</p>@enderror

                {{-- Region — hanya muncul jika role = aspapi_daerah --}}
                <div x-show="role === 'aspapi_daerah'" style="margin-top:1rem;">
                    ...
                    @error('region_id')<p style="font-size:0.75rem;color:#C0392B;margin-top:0.25rem;">{{ $message }}</p>@enderror
                </div>
            </div>

            {{-- Password --}}
            <div style="margin-bottom:1rem;">
                <label style="display:block;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#4A6580;margin-bottom:0.375rem;">Password</label>
                <input type="password" name="password" required minlength="8"
                       style="width:100%;padding:0.5rem 0.75rem;border:1.5px solid {{ $errors->has('password') ? '#C0392B' : '#D6E8F7' }};border-radius:4px;font-size:0.875rem;color:#1A2A3A;box-sizing:border-box;">
                @error('password')<p style="font-size:0.75rem;color:#C0392B;margin-top:0.25rem;">{{ $message }}</p>@enderror
            </div>

            {{-- Konfirmasi Password --}}
            <div style="margin-bottom:1.5rem;">
                <label style="display:block;font-size:0.7rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#4A6580;margin-bottom:0.375rem;">Konfirmasi Password</label>
                <input type="password" name="password_confirmation" required
                       style="width:100%;padding:0.5rem 0.75rem;border:1.5px solid #D6E8F7;border-radius:4px;font-size:0.875rem;color:#1A2A3A;box-sizing:border-box;">
            </div>
